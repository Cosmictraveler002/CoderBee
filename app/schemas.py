...
class ChatRequest(BaseModel):
    ...
    tempratures: float = Field(0.7, ge=0.1, le=1.5, description="Sampling tempratures")
...
class MemoryEntry(BaseModel):
    ...
    session_id: str = Field(..., description="Unique session id of the current chat")
    timestamp: str = Field(default_factory=lambda:datetime.now().isoformat(), description="ISO 8601 time format of the current message geenration")
